...
import com.teamsun.mapred.DqcMapper_LJ;
import com.teamsun.metadata.TableService;
/**
 * @author wpf
 *处理被压缩后的gz文件
 */
public class RunLJ_gz_utf8 {
	...
	public static void main(String[] args) {
		...
		interfaceCode=interfaceCode.substring(0, (interfaceCode.length())-2);
		...
		try {
			...
			status = new RunLJ_gz_utf8().run(col_Size,conf);
	...
	private int run(long cols,Configuration conf) throws IOException, ClassNotFoundException, InterruptedException{
		...
		job.setJarByClass(RunLJ_gz_utf8.class);
		job.setMapperClass(DqcMapper_LJ.class);
