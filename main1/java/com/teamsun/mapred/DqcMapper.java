...
//		String sv = value.toString();
		...
		if(this.matcher.find())
			sv=leftTrim(sv);
		//判断一行数据以-|-|结尾可以处理
		if((this.replaceRF(sv).contains("-|-|")) && (this.replaceRF(sv).split("\\|\\-\\|")[0].split("\001",Integer.MAX_VALUE).length == this.colnumSize)){
			String[] items = this.replaceRF(sv).split("\\|\\-\\|");
			for(String str:items){
				//调序后，直接写出
				val="";
				boolean isFirst = true;
				String[] items1 = str.split("\001",Integer.MAX_VALUE);
				for(String seq :seqs){
					if(isFirst){
						val=items1[Integer.parseInt(seq)];
						isFirst=false;
					}else {
						val+=Constants.HIVE_RF+items1[Integer.parseInt(seq)];
					}
				}
				mos.write(Constants.OUTPUTFILENAME, null, new Text(val));
//				context.getCounter("prov"+Constants.OUTPUTFILENAME, Constants.OUTPUTFILENAME).increment(1);
			}
			return;
		}
			...
			for(String id : Primary_Order_Id){
				if("".equals(val.split("\001",Integer.MAX_VALUE)[Integer.parseInt(id)-1].trim())){
					mos.write(Constants.PRIMARYKEYNULLNAME, null, new Text(
							(val.replaceAll(Constants.HIVE_RF,Constants.RF) + Constants.LF) 
							    + " "
	...
	public boolean judge_length(String col) throws IOException, InterruptedException{
		...
		//校验(CHAR,NCHAR,NVARCHAR,NVARCHAR2,VARCHAR,VARCHAR2，STRING,CLOB,LONG,alpha)验证数据长度---method=1
		if(method_code == 1){
		...
		if(method_code == 2){
			...
			//整数类型(包含有符号类型)匹配，如：number类型字段长度不包含刻度，(INT,INTEGER,NUMBER(p),NUMERIC,SMALLINT,TINYINT)
			else{
		...
		if(method_code == 3){
			Pattern pattern_3 = Pattern.compile("(^\\d{8}$)|(^\\d{2}-\\d{2}-\\d{4}$)" +
					...
					"|^14\\d{8}$|^$" +
					"|^$");
			if(!pattern_3.matcher(dll.trim()).find()){
