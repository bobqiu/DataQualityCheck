 ...
 * 
 */
public class DqcMapper_RunTxt extends Mapper<LongWritable, Text, Text, Text> {
	@Override
				...
				if("".equals(val.split("\001",Integer.MAX_VALUE)[Integer.parseInt(id)-1].trim())){
					mos.write(Constants.PRIMARYKEYNULLNAME, null, new Text(
							(val.replaceAll(Constants.HIVE_RF,Constants.RF) + Constants.LF) 
							    + " "
								+ "[the failed column is:" + (Integer.parseInt(id)) +"]," + "[fail data is:"+val.split(Constants.HIVE_RF,Integer.MAX_VALUE)[Integer.parseInt(id)-1].trim()+"],"
